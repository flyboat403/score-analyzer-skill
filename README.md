...
输出 JSON 包含：
- 动态及格线 (D-G = P20百分位数，超越后20%)
- 动态及格率 (理论≈80%)
- 相对优秀线 (D-E = P80百分位数，排进前20%)
- 绝对及格率/优秀率对比
- 各科目动态阈值
- 解读建议（题目极难时如何解释）
...
- ✓ 动态及格线基于P20百分位数（超越后20%），题目极难时替代绝对及格率
- ✓ 相对优秀线基于P80百分位数（排进前20%）
- ✓ 报告包含精细分数段统计
- ✓ 图表数量与占位符匹配（12张或9张）
- ✓ 单个图表文件大小 > 10KB（非空白/无效图片）
- ✓ HTML报告base64嵌入图片数量 ≥ 12（通过 `grep -c` 验证）
- ✓ Word文档实际嵌入图片数 ≥ 12（通过docx XML解析验证）
- ✓ 无原始占位符残留（`PLOT:XXX` 全部被替换）
- ✓ 个人报告数量与学生数匹配
- ✓ Word 报告文件大小 > 100KB
- ✓ 报告中提及的学生数/科目数与数据一致
